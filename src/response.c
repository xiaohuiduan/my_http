...
void my_ssl_write(SSL *ssl, char *buf, int num) {
    int ret = 0;
    int err = 0;
    while (ret <= 0) {
        sleep(1);
...
int send_ssl_response(struct http_response *response, struct my_socket *my_socket) {
    ...
    if (response->body != NULL) {
        if (CHUNK) {
            ...
            int i = 0;
            while (i < epoch) {
//                char *_chunk = (char *)malloc(8);
//                memset(_chunk, 0, 8);
//                snprintf(_chunk, 8, "%d\r\n", CHUNK_SIZE);
//                my_ssl_write(ssl, _chunk, 8);
                my_ssl_write(ssl, "2\r\n", 3);
...
//                free(_chunk);
            ...
            char *_chunk = (char *) malloc(8);
            snprintf(_chunk, 8, "%d\r\n", last);
            SSL_write(ssl, _chunk, 8);
            SSL_write(ssl, response->body + epoch * CHUNK_SIZE, last);
            SSL_write(ssl, "\r\n", 2);
            //            free(_chunk);
        } else {
